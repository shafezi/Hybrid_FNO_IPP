...
try:
    ...
    nc = None  # Not needed if using xarray
import xarray as xr
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]
        # Validate dimensions before FFT
        h, w = x.size(-2), x.size(-1)
        if h <= 0 or w <= 0:
            raise ValueError(f"Invalid spatial dimensions for FFT: {x.shape}. Dimensions must be positive.")
        
        # Ensure tensor is contiguous - cuFFT requires contiguous tensors
        if not x.is_contiguous():
            x = x.contiguous()
        
        # cuFFT can have issues with certain dimension sizes
        # Try to perform FFT, catch and report detailed error if it fails
        try:
            x_ft = torch.fft.rfft2(x)
        except RuntimeError as e:
            if "CUFFT_INVALID_SIZE" in str(e) or "cuFFT" in str(e):
                raise RuntimeError(
                    f"cuFFT error with input shape {x.shape} (spatial dims: {h}x{w}). "
                    f"This may be due to dimension size limits or invalid tensor format. "
                    f"Original error: {e}"
                ) from e
            raise
...
class FNO2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, '_debug_first_call'):
            if not self._debug_first_call:
                print(f"FNO forward input shape: {x.shape}")
                self._debug_first_call = True
        else:
            self._debug_first_call = False
            print(f"FNO forward input shape (first call): {x.shape}")
        ...
        # Debug: Check shape before conv0
        if not hasattr(self, '_debug_conv0_shape_printed'):
            print(f"Shape before conv0: {x.shape}")
            self._debug_conv0_shape_printed = True
...
def PECstep(net,input_batch,delta_t):
    ...
    '''
    # input_batch is already on DEVICE, no need for redundant .to(DEVICE) calls
    output_1 = net(input_batch) + input_batch 
    return input_batch + delta_t*0.5*(net(input_batch)+net(output_1))
...
def one_epoch(batch_size, optimizer, net, lambda_reg, lead, trainN, wavenum_init, wavenum_init_ydir, delta_t, timesteps = False):
    ...
    # Train on all years of data
    for k in range(1993, 2020): # loop through years of data
        
        # load data (try uppercase EC, fallback to lowercase)
        filename = data_dir + 'EnKF_surface_'+str(k)+'_5dmean_EC.nc'
        if not os.path.exists(filename):
            filename = data_dir + 'EnKF_surface_'+str(k)+'_5dmean_ec.nc'
        G = xr.open_dataset(filename)
        ...
        psi_train_label_Tr_torch_norm_level1 = ((psi_train_label_Tr_torch[:,0].to(DEVICE)-M_test_level1.to(DEVICE))/STD_test_level1.to(DEVICE))
        
        # Delete original tensors to free CPU memory
        del psi_train_input_Tr_torch, psi_train_label_Tr_torch
        
        # shuffle
        indices = torch.randperm(psi_train_label_Tr_torch_norm_level1.size(0))
        ...
        for step in range(0,trainN-batch_size,batch_size): # loop through batches
            ...
            if input_batch.size(2) <= 0 or input_batch.size(3) <= 0:
                print(f"ERROR: Invalid batch dimensions at step {step}, idxs={idxs}")
                print(f"input_batch shape: {input_batch.shape}")
                print(f"label_batch shape: {label_batch.shape}")
                continue

            # permute dimensions to match model (tensors already on DEVICE)
            input_batch = input_batch.permute(0,2,3,1)
            label_batch = label_batch.permute(0,2,3,1)
            
            # Debug: Print shape before FFT
            if step == 0:
                print(f"First batch shape after permute: {input_batch.shape}")

            # zero the parameter gradients
            optimizer.zero_grad()
            
            # forward + backward + optimize (input_batch already on DEVICE)
            output = PECstep(net, input_batch, delta_t)

            loss = spectral_loss_val(
                output,
                label_batch, 
                wavenum_init,wavenum_init_ydir,lambda_reg,
                torch.tensor(ocean_grid, device=DEVICE))
        ...
        # Clean up normalized tensors after processing each year
        del psi_train_input_Tr_torch_norm_level1, psi_train_label_Tr_torch_norm_level1
        torch.cuda.empty_cache()
...
def objective(trial):
    ...
    modes = 128             # for FNO
    width = 20              # for FNO
    wavenum_init = 90       # for spectral regularizer
    ...
    epochs_init = 180#180       # Full training epochs
    batch_size = 16#30         # Full batch size
    ...
    psi_test_input_Tr_torch = \
        ((psi_test_input_Tr_torch[:,:,:,:].to(DEVICE)-M_test_level1)/STD_test_level1)
    psi_test_label_Tr_torch = \
        ((psi_test_label_Tr_torch[:,:,:,:].to(DEVICE)-M_test_level1)/STD_test_level1)
    ...
    for epoch in range(0, epochs_init):
        ...
        with torch.no_grad():
            ...
            val_loss = spectral_loss_val(
                output,label,wavenum_init,wavenum_init_ydir,lambda_reg,
