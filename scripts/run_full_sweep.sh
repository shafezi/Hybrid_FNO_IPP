#!/bin/bash
# Full hyperparameter sweep with capped parallelism
# 4 kernels × 6 LS × 3 methods × 5 episodes = 360 runs
# Max 32 processes at a time, OMP=4 each

OUT=/tmp/full_sweep_10bots.csv
echo "t0,forecast_mode,kernel,nu,length_scale,noise_upper,RMSE,ACC,HF,FSS,time" > $OUT

# Generate all jobs as a list
JOBS=$(mktemp)
...
echo "Running with max 32 parallel, OMP=4 each"
echo "Expected time: ~$((TOTAL * 3 / 32)) min"

# Run with xargs -P 32
cat $JOBS | xargs -I {} -P 32 -d '\n' bash -c '
  IFS="|" read -r t0 fm kernel nu ls <<< "$1"
  OMP_NUM_THREADS=4 python scripts/run_gp_sweep.py \
    --t0 $t0 --n_robots 10 --forecast_mode $fm \
